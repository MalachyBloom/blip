...
def quickMapmaker(params, sample, parameters, inj, nside=32, saveto=None):
    '''
        This is a shortened version of the mapmaker in plotmaker.py. It only requires a sample (of blms) and some other basic information
        available in the run directory (easily obtained by the loadRunDir function). This returns a map of the gw power (a healpy array)
    ...
    if type(parameters) is dict:
        blm_start = len(parameters['noise']) + len(parameters['signal']) ## deal with extra parameter in broken_powerlaw:
...
def FWxM(skymap,fracMax=.5,nside=32,manual_peak_index=None,global_peak_val=None,ommission=[]):
    '''
        ...
        manual_peak_index : int

        global_peak_val : float
        ...
        spotSize : float
            Fraction of the sky that is contained 
    ...
    while not full:
        ...
        for pxl_index in border:
            if skymap[pxl_index] > fracMax*global_peak_val:
                ...
                count+=1
                full = False      
    spotSize = count/len(skymap) 
    return spotSize, signalBlob, peak_value, border
